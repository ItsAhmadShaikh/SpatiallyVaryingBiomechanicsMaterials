function [X,Y]=UniaxialTestFunction_Half(deleteFile,sampleDim,sampleMesh, E_ini, thickness_ini, strain, ratio, rho_mat)
...
%     clear; close all; clc;
    ...
    % Defining file names
    febioFebFileNamePart='InverseUniaxialModel';
    ...
    appliedStrain= strain/2; %Linear strain (Only used to compute applied stretch)
...
%     thickness_ini = [0 0.675 1.5]; %[0.15 0.6750 1.2750];
    ...
    E_youngs_layer = interp1(thickness_ini,E_ini,thicnkness_used,'pchip');

    nu1=0.45; %Material Poisson's ratio
    
%     rho_mat = 1.12; %Material Density
        
    cFigure; hold on;
    plot(thickness_ini,E_ini,'--k','lineWidth',lineWidth);
    plot(thicnkness_used,E_youngs_layer,'-o','lineWidth',lineWidth);
    xlabel('Thickness (mm)');
    ...
    E_temp = [];
    for k = 1:numElementsHeight_TotalLayer

        E_temp = [E_temp; ones(size(E_Layer,1),1)*E_youngs_layer(k)];
    end
    meshStruct.elementData= E_temp;
    %%
   ...
   febio_spec.Control.solver.symmetric_stiffness=0;
    ...
    %Material section
    % materialName={'Material1','Material2','Material3','Material4','Material5'};
    for k = 1:numElementsHeight_TotalLayer
        febio_spec.Material.material{k}.ATTR.name= ['Material',num2str(k)]; %char(materialName(k));
        febio_spec.Material.material{k}.ATTR.type='neo-Hookean';
        febio_spec.Material.material{k}.ATTR.id=k;
        febio_spec.Material.material{k}.E=E_youngs_layer(k);
        febio_spec.Material.material{k}.v=nu1;
        febio_spec.Material.material{k}.density=rho_mat;
    end
    ...
    for k = 1:numElementsHeight_TotalLayer
        febio_spec.Mesh.Elements{k}.ATTR.name= ['Part',num2str(k)];%char(partName(k)); %Name of this part
        febio_spec.Mesh.Elements{k}.ATTR.type='hex8'; %Element type

        %     febio_spec.Mesh.Elements{k}.elem.ATTR.id =(size(E_Layer,1)*(k-1))+(1:1:size(E_Layer,1))'; %Element id's
        E_id_end = E_id_end + size(E_Layer(:,:,k),1);
        febio_spec.Mesh.Elements{k}.elem.ATTR.id =(E_id_start:1:E_id_end)'; %Element id's
        E_id_start = E_id_end+1;

        febio_spec.Mesh.Elements{k}.elem.VAL = E_Layer(:,:,k); %The element matrix
    ...
    %MeshDomains section
    for k = 1:numElementsHeight_TotalLayer
        febio_spec.MeshDomains.SolidDomain{k}.ATTR.name = ['Part',num2str(k)];%char(partName(k));
        febio_spec.MeshDomains.SolidDomain{k}.ATTR.mat = ['Material',num2str(k)];%char(materialName(k));
    end
...
%     febio_spec.Boundary.bc{1}.dofs='x';
    ...
    febio_spec.Boundary.bc{1}.scale.VAL= -1*displacementMagnitude;
...
%     febio_spec.Boundary.bc{2}.dofs='y';
    if ratio ~= 0
        ...
        febio_spec.Boundary.bc{2}.scale.VAL= -1*displacementMagnitude*ratio;
...
%         febio_spec.Boundary.bc{3}.dofs='z';
    ...
    febio_spec.Boundary.bc{3}.ATTR.node_set=nodeSetName4;
    ...
    if ratio ~= 0
        ...
        febio_spec.Boundary.bc{4}.ATTR.node_set=nodeSetName5;
        febio_spec.Boundary.bc{4}.dof='y';
        febio_spec.Boundary.bc{4}.scale.ATTR.lc=1;
        febio_spec.Boundary.bc{4}.scale.VAL=displacementMagnitude*ratio;
...
%         DX_set=N_disp_mat(bcPrescribeList_X,1,:); %X displacements of the prescribed set
        DX_set=N_disp_mat(bcPrescribeList_X,1,:)*2; %X displacements of the prescribed set +  symmetry multiplication
        DX_set=mean(DX_set,1); %Calculate mean X displacements across nodes
        stretch_sim1=(DX_set(:)+sampleWidth)./sampleWidth; %Derive stretch
...
%         ylabel('$\sigma_{xx}$ [MPa]','FontSize',fontSize,'Interpreter','Latex');
        ...
        plot(stretch_sim1(:),stress_cauchy_sim(:),'r-','lineWidth',lineWidth);
...
%         DX_set=N_disp_mat(bcPrescribeList_X,1,:); %X displacements of the prescribed set
        DX_set=N_disp_mat(bcPrescribeList_X,1,:)*2; %X displacements of the prescribed set with symmetry
        DX_set=mean(DX_set,1); %Calculate mean X displacements across nodes
        stretch_sim2=(DX_set(:)+sampleWidth)./sampleWidth; %Derive stretch
...
%         ylabel('$\sigma_{yy}$ [MPa]','FontSize',fontSize,'Interpreter','Latex');
        ...
        plot(stretch_sim2(:),stress_cauchy_sim(:),'g-','lineWidth',lineWidth);
...
%         DX_set=N_disp_mat(bcPrescribeList_X,1,:); %X displacements of the prescribed set
        DX_set=N_disp_mat(bcPrescribeList_X,1,:)*2; %X displacements of the prescribed set with symmetry
        DX_set=mean(DX_set,1); %Calculate mean X displacements across nodes
        stretch_sim3=(DX_set(:)+sampleWidth)./sampleWidth; %Derive stretch
...
%         ylabel('$\sigma_{zz}$ [MPa]','FontSize',fontSize,'Interpreter','Latex');
        ...
        plot(stretch_sim3(:),stress_cauchy_sim(:),'b-','lineWidth',lineWidth);
...
%         ylabel('$\sigma_{VM}$ [MPa]','FontSize',fontSize,'Interpreter','Latex');
        ...
        plot(stretch_sim1(:),PVM(:),'k-','lineWidth',lineWidth);
...
%         X =(DX_set(:)+sampleWidth)./sampleWidth;
        X =(DX_set(:)*2+sampleWidth)./sampleWidth; %With symmetry
