function [meshStruct]=hexMeshBoxPerLayer(varargin)
...
nY=boxEl(2);
% nZ=boxEl(3);
nZ_layers = boxEl(3); %large Layers
perZ_layer = boxEl(4); %Layers per large layer

nZ = perZ_layer*nZ_layers;
...
for layer = 1:nZ_layers
    
    q_end = layer*perZ_layer;
    
    %Creating element definitions
%     q=(((layer*perZ_layer-1:(layer*perZ_layer-1 + perZ_layer - 1))-1).*size(V2,1));

    q=(((q_start:q_end)-1).*size(V2,1));
    Q=q(ones(size(F2,1),1),:);
    Qc=Q(:);
    QQc=Qc(:,ones(1,size(F2,2)));

    
    q=((q_start:q_end).*size(V2,1));
    Q=q(ones(size(F2,1),1),:);
    Qc=Q(:);
    ...
    E_layer(:,:,layer)=[repmat(F2,[perZ_layer 1])+(QQc) repmat(F2,[perZ_layer 1])+(QQc2); ];
...
switch outputStructType
    ...
    case 2
        ...
        meshStruct.elements_layer=E_layer;
